  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        >
          <View style={styles.statusContent}>
            ...
            <View style={styles.statusIconContainer}>
              <Ionicons name={getStatusIcon(booking.status)} size={24} color={COLORS.white} />
            </View>
          </View>
          {booking.status === 'pending' && (
            <View style={styles.statusMessage}>
              <Ionicons name="alert-circle" size={16} color="rgba(255,255,255,0.8)" />
              <Text style={styles.statusSub}>Action required: Accept or Decline</Text>
            </View>
          )}
        </LinearGradient>
        ...
        {/* Handover Details */}
        {(booking.pickupImages?.length > 0 || booking.returnImages?.length > 0) && (
          <View style={styles.section}>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.handoverSection}>
                ...
                {booking.pickupImages?.length > 0 && (
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.miniGallery}>
                    {booking.pickupImages.map((img, i) => (
                      <Image key={i} source={{ uri: img }} style={styles.miniThumb} />
                    ))}
                  </ScrollView>
                )}
              </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.carCard}>
            ...
            />
            <LinearGradient
              colors={['transparent', 'rgba(15, 33, 55, 0.95)', COLORS.navy[800]]}
          ...
          <View style={styles.card}>
            <View style={styles.renterHeader}>
              ...
              <TouchableOpacity style={styles.actionIconBtn}>
                 <Ionicons name="mail-outline" size={20} color={COLORS.gold[500]} />
              </TouchableOpacity>
            </View>
          </View>
        </View>
          ...
          <View style={styles.card}>
            <View style={styles.timelineItem}>
             ...
             <View style={styles.timelineItem}>
              ...
              <View style={styles.timelineCenter}>
                <View style={[styles.dot, { backgroundColor: COLORS.red[500], marginTop: -2 }]} />
              </View>
              <View style={styles.timelineRight}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            <View style={styles.row}>
              ...
              <Text style={styles.rowValue}>PKR {Number(booking.totalPrice).toLocaleString()}</Text>
            </View>
             <View style={styles.row}>
              <Text style={styles.rowLabel}>Service Fee ({booking.platformCommissionPercent || 10}%)</Text>
              <Text style={[styles.rowValue, { color: COLORS.red[500] }]}>
                - PKR {Number(booking.platformCommissionAmount || 0).toLocaleString()}
              </Text>
            </View>
        ...
        <View style={styles.footer}>
          ...
          >
            <LinearGradient colors={[COLORS.emerald[500], '#34D399']} style={styles.gradientBtn}>
              ...
              <Text style={[styles.acceptText, { color: COLORS.white, marginLeft: 8 }]}>Scan QR to Start</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
        ...
        <View style={styles.footer}>
          ...
          >
            <LinearGradient colors={[COLORS.gold[500], '#FBBF24']} style={styles.gradientBtn}>
              ...
              <Text style={[styles.acceptText, { marginLeft: 8 }]}>Scan QR to Complete</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
...
const getStatusGradient = (status) => {
  switch (status) {
    case 'confirmed': return [COLORS.emerald[500], COLORS.emerald[400]];
    case 'pending': return [COLORS.gold[500], COLORS.gold[500]]; // Flat gold or slight gradient
    case 'ongoing': return [COLORS.navy[700], COLORS.navy[600]];
    case 'completed': return [COLORS.gray[500], COLORS.gray[400]];
    case 'cancelled': return [COLORS.red[500], COLORS.red[500]];
    default: return [COLORS.navy[800], COLORS.navy[700]];
  }
};
...
const styles = StyleSheet.create({
  ...
  backBtn: { width: 40, height: 40, backgroundColor: COLORS.navy[800], borderRadius: 12, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: COLORS.navy[700] },

  scroll: { padding: 20, paddingBottom: 120 },

  // Sections
  section: { marginBottom: 28 },
  sectionTitle: { color: COLORS.gray[400], fontSize: 13, fontWeight: '700', marginBottom: 12, letterSpacing: 0.5 },

  // Generic Card
  card: { backgroundColor: COLORS.navy[800], borderRadius: 20, padding: 16, borderWidth: 1, borderColor: COLORS.navy[700] },
  ...
  statusLabel: { color: 'rgba(255,255,255,0.7)', fontSize: 12, fontWeight: '700', marginBottom: 4, letterSpacing: 1 },
  statusValue: { fontSize: 26, fontWeight: '800', color: COLORS.white, letterSpacing: -0.5 },
  statusIconContainer: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center' },
  statusMessage: { flexDirection: 'row', alignItems: 'center', marginTop: 16, backgroundColor: 'rgba(0,0,0,0.1)', padding: 10, borderRadius: 8, gap: 8 },
  statusSub: { color: 'rgba(255,255,255,0.9)', fontSize: 13, fontWeight: '500' },

  // Handover Card
  handoverSection: { marginBottom: 4 },
  handoverHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  handoverInfo: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  handoverTitle: { color: COLORS.white, fontSize: 14, fontWeight: '600' },
  statusBadgeSuccess: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(16, 185, 129, 0.1)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  statusBadgePending: { backgroundColor: 'rgba(156, 163, 175, 0.1)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  badgeText: { color: COLORS.emerald[500], fontSize: 11, fontWeight: '700' },
  badgeTextPending: { color: COLORS.gray[400], fontSize: 11, fontWeight: '600' },
  miniGallery: { flexDirection: 'row', marginTop: 4, marginBottom: 8 },
  miniThumb: { width: 60, height: 60, borderRadius: 8, marginRight: 8, borderWidth: 1, borderColor: COLORS.navy[600] },
  cardDivider: { height: 1, backgroundColor: COLORS.navy[700], marginVertical: 16 },

  // Car Card
  carCard: { borderRadius: 20, overflow: 'hidden', backgroundColor: COLORS.navy[800], borderWidth: 1, borderColor: COLORS.navy[700] },
  carImageLarge: { width: '100%', height: 180 },
  carOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, paddingTop: 40 },
  carName: { color: COLORS.white, fontSize: 20, fontWeight: '800', marginBottom: 4 },
  carMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  carYear: { color: COLORS.gray[400], fontSize: 14, fontWeight: '500' },
  plateTag: { backgroundColor: COLORS.gold[500], paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  plate: { color: COLORS.navy[900], fontSize: 11, fontWeight: '700' },

  // Renter Card
  renterHeader: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  avatarContainer: { width: 56, height: 56, borderRadius: 28, backgroundColor: COLORS.navy[700], justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: COLORS.navy[600] },
  avatarText: { color: COLORS.white, fontSize: 24, fontWeight: '600' },
  renterName: { color: COLORS.white, fontSize: 16, fontWeight: '700', marginBottom: 2 },
  ...
  actionIconBtn: { width: 44, height: 44, borderRadius: 12, backgroundColor: 'rgba(245, 158, 11, 0.1)', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(245, 158, 11, 0.3)' },

  // Timeline
  timelineItem: { flexDirection: 'row', height: 70 },
  timelineLeft: { width: 70, alignItems: 'flex-end', paddingRight: 10 },
  timelineTime: { color: COLORS.white, fontSize: 14, fontWeight: '600', marginBottom: 2 },
  ...
  // Footer
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: COLORS.navy[900], padding: 20, paddingBottom: 30, borderTopWidth: 1, borderTopColor: COLORS.navy[700], flexDirection: 'row', gap: 15 },
  declineBtn: { flex: 1, height: 50, borderRadius: 14, borderWidth: 1, borderColor: COLORS.red[500], justifyContent: 'center', alignItems: 'center' },
